class Sphere {
    ...
    render (gl, program, flatOrRound) {
        ...
        for (var i = 0; i < this.faces.length; i++) {
            ...
            gl.uniform4fv(color2Loc, this.color.flatten());

            gl.uniform4fv(normalLoc, this.face_normals[i].flatten());
            if (this.color == new PV && this.faces[i].length > 4)
    ...
    contains (f, p) {
        var n = this.face_normals[f];
        var size = this.verts[this.faces[f][0]].length;
        for (var i = 0; i < size; i++){

            var vi = this.verts[this.faces[f][i]];
            var vi1 = this.verts[this.faces[f][(i+1)%size]];
            var cross = (vi.minus(p)).cross(vi1.minus(p));
